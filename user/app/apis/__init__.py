...
from tools.auth import TokenAuthError, PasswordAuthError,\
    WWW_AUTH_TOKEN_HEADER, WWW_AUTH_HEADER_PASSWORD, authorizations
...
def failed_token_auth_response(error):
    ...
    return ({'message': error.message}, error.code,
            {'WWW-Authenticate': WWW_AUTH_TOKEN_HEADER})
...
def failed_password_auth_response(error):
    ...
    return ({'message': error.message}, error.code,
            {'WWW-Authenticate': WWW_AUTH_HEADER_PASSWORD})
