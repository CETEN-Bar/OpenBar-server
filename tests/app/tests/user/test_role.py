...
def test_add():
    """Test adding roles"""
    tool_test_add(API_ROOT, is_role_equal, EXAMPLE_MINIMAL,
                  EXAMPLE_FULL, random_role)
...
def test_not_exist():
    ...
    for _ in range(10):
        ...
        rolelist.append(add_obj_and_verify(API_ROOT, is_role_equal,
                                           random_role()))
...
def test_readonly_attribute():
    ...
    for _ in range(10):
        rolelist.append(add_obj_and_verify(API_ROOT, is_role_equal,
                                           random_role()))
        for id in (rolelist[random.randint(0, len(rolelist) - 1)]['id'],
...
def test_wrong_type():
    ...
    def update_add_fail(role):
        ...
        is_role_list_equal(rolelist)
        if len(rolelist) != 0:
            assert requests.put(API_ROOT + str(rolelist[ind]['id']),
                                json=role) \
                .status_code in (400, 500)
        is_role_list_equal(rolelist)
    for _ in range(10):
