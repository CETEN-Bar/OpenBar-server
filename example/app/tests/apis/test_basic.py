...
def get_all_one_equivalent(client):
    ...
    """
    res = client.get(url_for('apis.basic_task_list'))
    assert res.status_code == 200
    for task in res.json:
        res = client.get(url_for('apis.basic_task', id=task['id']))
        assert res.status_code == 200
...
def add_task_and_verify(client, task):
    ...
    res =  client.post(url_for('apis.basic_task_list'),
            json=task)
    assert res.status_code == 201
    task_resulting = res.json
    for key in task:
        assert task_resulting[key] == task[key]
...
def delete_task_and_verify(client, task_id):
    ...
    assert client.delete(url_for('apis.basic_task',
            id=task_id),
        ).status_code == 204
...
def update_task_and_verify(client, task_id, task_update):
    ...
    res = client.get(url_for('apis.basic_task', id=task_id))
    assert res.status_code == 200
    actual_task = res.json

    # update
    res = client.put(url_for('apis.basic_task', id=task_id),
            json=task_update)
    assert res.status_code == 200
...
def test_empty(client):
    ...
    """
    assert client.delete(url_for('apis.basic_task_list')).status_code == 204
    res = client.get(url_for('apis.basic_task_list'))
    assert res.status_code == 200
    assert res.json == []
...
def test_not_exist(client):
    ...
    for _ in range(10):
        ...
        for _ in range(10):
            assert client.get(url_for('apis.basic_task',
                id=random.randint(themax, themax + 20))
                ).status_code == 404
        for _ in range(10):
            assert client.put(url_for('apis.basic_task',
                id=random.randint(themax, themax + 20)),
...
def test_add_required_args(client):
    ...
    for _ in range(5):
        tasklist.append(add_task_and_verify(client, random_task()))
        assert client.post(url_for('apis.basic_task_list')
            ).status_code == 400
        assert client.post(url_for('apis.basic_task_list'),
            json={}
            ).status_code == 400
...
def test_readonly_attribute(client):
    ...
    for _ in range(10):
        ...
        task = random_task()
        task['id'] = tasklist[random.randint(0, len(tasklist) - 1)]['id']
        res = client.put(url_for('apis.basic_task', id=tasklist[ind]['id']),
            json=task)
        assert res.status_code == 200
        assert res.json['id'] == tasklist[ind]['id']
        tasklist[ind] = res.json
...
def test_wrong_type(client):
    ...
    def update_add_fail(task):
        ...
        assert client.post(url_for('apis.basic_task_list'), json=task
            ).status_code in (400, 500)
        tasklist_ok(client, tasklist)
        if len(tasklist) != 0:
            assert client.put(url_for('apis.basic_task', id=tasklist[ind]['id']), json=task
                ).status_code in (400, 500)
        tasklist_ok(client, tasklist)
    for _ in range(10):
        if len(tasklist) != 0:
